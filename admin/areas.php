...
if (isset($_GET['id']) && checkFeild($_GET['id'])) {
	...
	if (isset($_GET['delete'])) {
		if($db->query("DELETE FROM areas WHERE id='$id' LIMIT 1")) {
			$_SESSION['msg'] = "Area successfully deleted.";
			$_SESSION['error'] = false;
			header("Location: areas.php");
			exit();
		} else {
			$_SESSION['msg'] = "Area can't be deleted.";
			$_SESSION['error'] = true;
			header("Location: areas.php");
			exit();
		}
		...
		if ($db->query_update("areas", $data, "id='$id'")) {
			$_SESSION['msg'] = "Area status successfully changed.";
			$_SESSION['error'] = false;
			header("Location: areas.php");
			exit();
		} else {
			$_SESSION['msg'] = "Area status can't be changed.";
			$_SESSION['error'] = true;
			header("Location: areas.php");
			exit();
		}
                    ...
                    <div class="row-fluid sortable">		
                        <div class="box span12">
                            <div class="box-header" data-original-title>
                                <h2><i class="icon-map-marker"></i><span class="break"></span>Areas</h2>
                                <div class="box-icon">
                                    <a href="areas-add.php" class="btn-link noUnderline"><i class="fa-icon-plus"> Add New Area</i></a>
                                </div>
                            </div>
                            ...
                            <div class="box-content">
                                <table class="table table-striped table-bordered bootstrap-datatable datatable">
                                  ...
                                  <tbody>
                                    <?php $query = $db->query("SELECT * FROM areas ORDER BY id"); ?>
                                    <?php if ($db->affected_rows > 0) { ?>
										<?php while($r=$db->fetch_array($query)) { ?>
                                            <tr>
                                                <td><?php echo $r['title']; ?></td>
                                                <td><?php echo $r['title_cn']; ?></td>
                                                <td><?php echo recordsCounter('buildings','area_id',$r['id']); ?></td>
                                                ...
                                                <td class="center">
                                                    <a class="btn btn-info" href="areas-edit.php?id=<?php echo $r['id']; ?>" title="Edit">
                                                        <i class="icon-edit icon-white"></i>  
                                                    </a>
                                                    <a class="btn btn-inverse" href="?id=<?php echo $r["id"]; ?>&status=<?php echo $r['status']==1 ? 0:1; ?>" title="Change English Status">
                                                        <i class="icon-eye-<?php echo $r['status']==1 ? 'open':'close'; ?> icon-white"></i> 
                                                    </a>

                                                    <a class="btn btn-warning" href="?id=<?php echo $r["id"]; ?>&status_cn=<?php echo $r['status_cn']==1 ? 0:1; ?>" title="Change Chinese Status">
