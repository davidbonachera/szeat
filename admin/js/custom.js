...
function charts(orders, users) {
	...
	{
		if (orders==false) {
			var orders = [[1, 5], [2, 10], [3, 15], [4, 20],[5, 25]];
		}
		if (users==false) {
			var users = [[1, 1], [2, 7], [3, 12], [4, 17],[5, 21]];
		}

		var plot = $.plot($("#stats-chart"),
			   [ { data: orders, label: "Orders"}, { data: users, label: "Users" } ], {
				   series: {
					   lines: { show: true,
								lineWidth: 3,
			...
			$("#y").text(pos.y.toFixed(2));
				...
				if (item) {
					if (previousPoint != item.dataIndex) {
						...
						showTooltip(item.pageX, item.pageY, Math.round(y)+" " +item.series.label+" on "+Math.round(x));
					}
				}
				else {
					$("#tooltip").remove();
